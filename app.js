function App() {
  ...
  const highPriorityCount = boards.reduce((total, board) => {
    return (
      total + board.tasks.filter((task) => task.priority === "high").length
    );
  }, 0);
  ...
  function reorderTasks(draggedId, targetId, columnName) {
    setBoards((prevBoards) =>
      prevBoards.map((b) => {
        ...
        const columnTasks = b.tasks.filter((t) => t.status === columnName);
          ...
          return (
            ...
            >
              ...
              {tasksInColumn.map((task) => {
                const isOverdue =
                  task.dueDate && new Date(task.dueDate) < new Date(); // Checks if the task is overdue by comparing due date to today
                ...
                return (
                  <div
                    ...
                    draggable
                    onDragStart={(
                      e, // Stores the task ID when dragging starts so it can be moved later
                    ) => e.dataTransfer.setData("taskId", String(task.id))}
                    //Allows the element to accept dropped items and handles dropping a draged task
                    //to reorder it.
                    onDragOver={(e) => e.preventDefault()}
                    onDrop={(e) => {
                      // Allows dropping a task into a column and updates its status
